// Adapted from SAMKit by john-rocky.
...
final class PromptEncoder {
  ...
  init(weightsURL: URL) throws {
    ...
    let h = ies[0]
    let w = ies[1]
    let ed = self.embedDim
    let dense = try MLMultiArray(
      shape: [1, ed as NSNumber, h as NSNumber, w as NSNumber], dataType: .float32)
